...
std::ostream& operator<<(std::ostream& os, const Position& pos) {
      ...
      && !pos.can_castle(ANY_CASTLING))
  {
      ...
      os << "\nSYZYGY-Tablebases WDL: " << std::setw(4) << wdl << " (" << s1 << ")"
         << "\nSYZYGY-Tablebases DTZ: " << std::setw(4) << dtz << " (" << s2 << ")";
  }
#endif

#ifdef LOMONOSOV_TB
  if (Tablebases::max_tb_pieces >= popcount(pos.pieces())
	  && !pos.can_castle(ANY_CASTLING))
  {
	  StateInfo st;
	  Position p;
	  p.set(pos.fen(), pos.is_chess960(), &st, pos.this_thread());
	  int value; bool dtm;
	  bool result = lomonosov_tbprobe(p, 0, &value, false, 0, &dtm);
	  os << "\nLomonosov-Tablebases: " << std::setw(4) << value << " (" << result << (dtm ? "DTM" : "WDL") << ")";
  }
#endif
...
void Position::lomonosov_position(int *side, unsigned int *psqW, unsigned int *psqB, int *piCount, int *sqEnP) {
	*side = sideToMove;
	...
	for (int i = 0; i < 10; i++) {
		...
		piCount[i] = pieceCount[make_piece(color, pcType)];
		if (i < 5)
			psq_tmp = psqW;
		else
			psq_tmp = psqB;
		for (int j = 0; j < piCount[i]; j++) {
			psq_tmp[(i % 5)*C_PIECES + j] = pieceList[make_piece(color, pcType)][j];
		}
	}
	psqW[KING_INDEX] = pieceList[make_piece(WHITE, KING)][0];
	psqB[KING_INDEX] = pieceList[make_piece(BLACK, KING)][0];
	*sqEnP = ep_square();
}
#endif
