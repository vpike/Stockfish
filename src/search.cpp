...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
	...
	if (lomonosov_probe) {
		int v, wdl;
        bool from_dtm;
		if ((lomonosov_success = lomonosov_tbprobe(pos, ss->ply, &v, true, thisThread->idx, &from_dtm, &wdl))) {
			value = (Value)v;
#ifdef SYZYGY_TB
            ...
            Bound b = (from_dtm || wdl == 0) ? BOUND_EXACT
                    : wdl < 0 ? BOUND_UPPER : BOUND_LOWER;
...
void Tablebases::filter_root_moves(Position& pos, Search::RootMoves& rootMoves) {
	...
	root_is_win_dtm = false;
	if (use_tables && (max_tb_pieces >= popcount(pos.pieces()) && !pos.can_castle(ANY_CASTLING))) {
		if (lomonosov_root_probe(pos, rootMoves, &root_is_win_dtm)) {
			RootInTB = true;
			if (!root_is_win_dtm) {
